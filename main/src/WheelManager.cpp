...
{
    wheel_container_ = std::make_unique<WheelContainer>();
    
    TaskManagerConfig task_config{
        .control_loop_frequency = config.control_loop_frequency,
        .odo_broadcast_frequency = config.odo_broadcast_frequency
    };
    
    task_manager_ = std::make_unique<TaskManager>(task_config, *wheel_container_);
}
